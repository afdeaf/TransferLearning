...
class BaseTrainer(object):
    ...
    def test(self):
        ...
        tar_acc = self.test_func(self.dataset_loaders['target_test'], self.base_net, domain='tar')
        is_best = False
    ...
    def test_func(self, loader, model, domain='src'):
        with torch.no_grad():
            ...
            for i in range(len(loader)):
                ...
                if domain == 'src':
                    outputs = self.fc(self.w * outputs_all)
                else:
                     outputs = self.fc(outputs_all)
